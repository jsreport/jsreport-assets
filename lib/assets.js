...
function resolveAssetLink (reporter, req, assetName) {
  if (reporter.options.assets.rootUrlForLinks) {
    return url.resolve(reporter.options.assets.rootUrlForLinks, 'assets/' + encodeURIComponent(assetName))
  }

  if (!req.url) {
    var protocol = reporter.options.httpPort ? 'http://' : 'https://'
    var port = reporter.options.httpPort || reporter.options.httpsPort
    return url.resolve(protocol + 'localhost:' + port, 'assets/' + encodeURIComponent(assetName))
  }

  var base = req.protocol + '://' + req.get('host')
  return base + (req.originalUrl || '/').replace('api/report', '') + 'assets/' + encodeURIComponent(assetName)
}
...
module.exports = function (reporter, definition) {
  ...
  reporter.on('express-configure', function (app) {
    ...
    app.get('/assets/:name', function (req, res) {
      readAsset(reporter, req.params.name, 'binary', req).then(function (asset) {
