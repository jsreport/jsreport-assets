...
function evaluateAssets (reporter, stringToReplace, req) {
  ...
  function convert (str, p1, offset, s, done) {
    ...
    reporter.documentStore.collection('assets').find({ name: assetName }, req).then(function (result) {
      ...
      if (result[0].link) {
        return new Buffer(readFile(reporter, result[0].link)).toString(encoding)
      }
...
function linkPath (reporter, link) {
  ...
  if (!reporter.options.assets || !minimatch(result, reporter.options.assets.allowedFiles) || !minimatch(link, reporter.options.assets.allowedFiles)) {
    var err = new Error('Request to file ' + result + ' denied. Please allow it by setting config { "files": { "allowedFiles": "**/foo.js" } }')
    err.weak = true
    throw err
  }
