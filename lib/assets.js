  ...
  function convert (str, p1, offset, s, done) {
    ...
    if (p1.indexOf(' @') !== -1) {
      ...
      if (paramValue !== 'base64' && paramValue !== 'utf8' && paramValue !== 'string' && paramValue !== 'link' && paramValue !== 'dataURI') {
        throw new Error('Unsupported asset encoding param value ' + paramValue + ', supported values are base64, utf8, dataURI and string')
      }

      if (paramValue === 'dataURI' && !isImage(assetName)) {
        throw new Error('Asset encoded as dataURI needs to have file extension jpeg|jpg|gif|png|svg')
      }
...
function readAsset (reporter, name, encoding, req) {
  ...
  if (encoding === 'dataURI') {
    escape = function (val, name) {
      ...
      return 'data:image/' + fileExtension + ';base64,' + val
    }
    encoding = 'base64'
  }
