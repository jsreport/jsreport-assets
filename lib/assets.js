...
async function readAsset (reporter, definition, id, name, encoding, req) {
  ...
  let asset
  if (id) {
    asset = await reporter.documentStore.collection('assets').findOne({ _id: id }, definition.options.publicAccessEnabled ? null : req)
    ...
    const assets = await reporter.documentStore.collection('assets').find(folderQuery, definition.options.publicAccessEnabled ? null : req)
    ...
    // falback to global search
      ...
      const assets = await reporter.documentStore.collection('assets').find(globalQuery, definition.options.publicAccessEnabled ? null : req)
