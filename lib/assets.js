...
function resolveAssetLink (reporter, req, assetName) {
  if (reporter.options.assets.rootUrlForLinks) {
    return url.resolve(reporter.options.assets.rootUrlForLinks, 'assets/content?name=' + encodeURIComponent(assetName))
  }

  if (!req.url) {
    var protocol = reporter.options.httpPort ? 'http://' : 'https://'
    var port = reporter.options.httpPort || reporter.options.httpsPort
    return url.resolve(protocol + 'localhost:' + port, 'assets/content?name=' + encodeURIComponent(assetName))
  }

  var base = req.protocol + '://' + req.get('host')
  return base + (req.originalUrl || '/').replace('api/report', '') + 'assets/content?name=' + encodeURIComponent(assetName)
}
  ...
  return reporter.documentStore.collection('assets').find({ name: name }, reporter.options.assets.publicAccessEnabled ? null : req).then(function (result) {
    if (result.length < 1) {
      ...
      return readFile(reporter, name).then(function (res) {
        return {
          content: escape(new Buffer(res.content).toString(encoding)),
    ...
    return {
      content: escape(new Buffer(result[0].content).toString(encoding)),
...
module.exports = function (reporter, definition) {
  reporter.documentStore.registerEntityType('AssetType', {
    ...
    modificationDate: { type: 'Edm.String' },
  ...
  reporter.on('express-configure', function (app) {
    ...
    app.get('/assets/content', function (req, res) {
      readAsset(reporter, req.query.name, 'binary', req).then(function (asset) {
    ...
    app.get('/assets/link', function (req, res) {
      try {
        res.send(linkPath(reporter, req.query.name))
      } catch (e) {
        res.status(500).end(e.message)
      }
    })
