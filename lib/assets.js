...
async function readAsset (reporter, definition, name, encoding, req) {
  ...
  const assets = await reporter.documentStore.collection('assets').find({$or: [{ name: name }, { link: name }]}, definition.options.publicAccessEnabled ? null : req)
  if (assets.length < 1) {
  ...
  if (encoding === 'link') {
    return assets[0].link ? {
      content: resolveAssetLink(reporter, definition, req, assets[0].link),
      filename: name
    } : {
      content: resolveAssetLink(reporter, definition, req, name),
      filename: name
  ...
  if (assets[0].link) {
    const file = await readFile(reporter, definition, assets[0].link)
    return {
      content: escape(Buffer.from(file.content).toString(encoding), file.filename),
      filename: file.filename,
      modified: file.modified
...
module.exports = function (reporter, definition) {
  ...
  if (reporter.options.renderingSource === 'trusted' && !definition.options.allowedFiles && definition.optoins.searchOnDiskIfNotFoundInStore == null) {
    ...
    reporter.documentStore.collection('assets').beforeUpdateListeners.add('assets', async (query, update) => {
      ...
      if (query._id && update.$set && update.$set.forceUpdate && update.$set.link) {
        try {
          ...
          const error = new Error('Unable to access file ' + linkPath(reporter, definition, update.$set.link))
          error.weak = true
          throw error
        }
      } else {
