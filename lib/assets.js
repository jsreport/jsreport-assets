...
function resolveAssetLink (reporter, req, assetName) {
  ...
  var base = req.protocol + '://' + req.get('host') + reporter.options.appPath

  if (base[base.length - 1] === '/') {
    base = base.substring(0, base.length - 1)
  }

  return base + '/assets/content/' + assetName
}
...
module.exports = function (reporter, definition) {
  ...
  reporter.beforeRenderListeners.insert({ after: 'scripts' }, definition.name, this, function (req, res) {
    return reporter.documentStore.collection('assets').find({ isSharedHelper: true }, req).then(function (assets) {
      ...
      // don't add the shared helpers for child requests from phantom headers
      // (helpers are already there from previous execution of main template)
      // but add them for request from child templates
      if (req.options.isChildRequest === true && !req.options.isChildTemplateRequest) {
        return
      }
      ...
      return Promise.map(assets, function (a) {
        ...
        assetContents.forEach(function (ac) {
          req.template.helpers = (req.template.helpers == null ? '' : req.template.helpers) + '\n' + ac.content
        })
      })
    }).then(function () {
